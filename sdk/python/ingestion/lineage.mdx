...
description: Learn to implement data lineage tracking with Collate's Python SDK. Complete guide with code examples, API usage, and best practices for data flow management.
...
<Warning>
Lineage operations (`add_lineage`, `get_lineage_by_name`, `add_lineage_by_query`) are not yet available in the Fluent API. They will be added in a future SDK release. Track progress in the [SDK changelog](/sdk/python/fluent).
</Warning>

With everything prepared, you can create Lineage between both Entities once fluent lineage support is released. An `AddLineageRequest` type represents the edge between two Entities, typed under `EntitiesEdge`. The request structure looks like this:
...
Fetching lineage by name (`get_lineage_by_name`) is also pending the fluent API. The method signature for reference:

```python
# Pending fluent API — signature reference only
# Tables.get_lineage_by_name(
...
# Pending fluent API: created_lineage = Tables.add_lineage(add_lineage_request)
```
...
# Note: PipelineServices.create() is pending the fluent API.
# Once available, replace with: pipeline_service_entity = PipelineServices.create(pipeline_service)

create_pipeline = CreatePipelineRequest(
    name="test",
    service="test-service-pipeline",  # use the service name directly
)
...
database_service = DatabaseServices.retrieve_by_name("my_service")

# Pending fluent API: Tables.add_lineage_by_query(
#     database_service=database_service,
#     timeout=200,
#     sql="insert into target_table(id) as select id from source_table",
# )
```
