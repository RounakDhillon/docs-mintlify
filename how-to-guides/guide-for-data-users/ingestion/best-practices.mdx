...
description: Discover Collate data ingestion best practices to optimize your metadata collection, improve performance, and ensure reliable data connectivity.
slug: /how-to-guides/guide-for-data-users/ingestion/best-practices
...
- **DAGs should not have any retry**: If the workflow is marked as failed due to any error (unexpected exception,
    ...
    Note that for internal communications between the Ingestion Workflow and the Collate APIs, we already have an
...
- **Connection errors**: When deploying ingestions from the Collate UI you have the possibility to test the
