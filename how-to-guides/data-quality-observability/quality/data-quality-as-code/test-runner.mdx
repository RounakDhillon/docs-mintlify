...
- [Running Tests from Collate's UI](#running-tests-from-openmetadata-ui)
...
Instead of defining tests in code, you can run tests that data stewards have configured in the Collate UI. This enables a collaborative workflow where:
...
**Collate's ingestion extras**: `aws` (e.g `pip install 'openmetadata-ingestion[aws]'`)
...
**Collate's ingestion extras**: `azure` (e.g `pip install 'openmetadata-ingestion[azure]'`)
...
**Collate's ingestion extras**: `gcp` (e.g `pip install 'openmetadata-ingestion[gcp]'`)
