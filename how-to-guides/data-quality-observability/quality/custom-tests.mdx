---
title: Custom Tests | Collate Quality Testing Guide
...
sidebarTitle: Custom Tests
---

# Adding Custom Tests
While Collate provides out of the box tests, you may want to write your test results from your own custom quality test suite or define your own data quality tests to be ran inside Collate.  This is very easy to do using the API and our Python SDK.
...
**Important:** If you want to have the test definition available through Collate UI, `testPlatforms` need to include `Collate`. It will also require extra work that we'll cover below in Step 5. If you are just looking to create new test definition executable through Collate UI, you can skip ahead to Step 5.
...
### Step 4: Writing `TestCaseResults` (Optional - if not executing test case through Collate UI)
...
### Step 5: Making Custom Test Case Available Through Collate UI (Optional)
Collate offers the flexibility to user to create custom test cases that will be executable through the user interface. To accomplish our goal, we'll be leveraging Collate namespace `data_quality` submodule .
...
Once you have created the different, you can add the logic in your `<yourTest>.py` file. You will need to create a class named `<YourTest>Validator` that will inherit from `BaseTestValidator`. If you need to, you can also inherit from `SQAValidatorMixin` -- this will give you access to additional methods out of the box. Once completed, you will simply need to implement the `run_validation` class. This method should return a `TestCaseResult` object. You can find a full implementation [here](https://github.com/open-metadata/Collate-demo/tree/main/custom-om-test) where we create an entropy test.
...
Once you have completed A) and B) you should only need to `pip install` your package in the environment where Collate python SDK is install.

<img noZoom src="/public/images/features/ingestion/workflows/data-quality/custom-test-definition.png" alt="Create test case" />

 <img noZoom src="/public/images/features/ingestion/workflows/data-quality/custom-test-result.png" alt="Create test case" />