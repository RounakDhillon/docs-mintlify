...
description: Resolve glossary CSV export failures in Collate by checking proxy settings, enabling WebSockets, and verifying real-time connectivity.
slug: /how-to-guides/data-discovery/troubleshooting
sidebarTitle: Import-Export Troubleshooting
---
...
This issue may occur if **WebSockets are blocked** in your network setup due to a **proxy** or **load balancer** configuration. Collate relies on WebSockets for real-time communication, and if they are blocked, the export process cannot complete.
...
4. Check if WebSocket requests to Collate (wss://&lt;your-openmetadata-instance&gt;) are blocked, failing, or not established.
...
3. Retry the CSV export in Collate.
