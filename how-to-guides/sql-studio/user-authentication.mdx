...
- For Basic Authentication: You need your database credentials (Service Account JSON, Username + Password, or Key Pair)
...
<img src="/public/images/how-to-guides/sql_studio/sql_studio_welcom.png" alt="SQL Studio Landing Page" />
...
<img src="/public/images/how-to-guides/sql_studio/user_sso_or_oauth_connect_button.png" alt="Connect to BigQuery Dialog" />
...
<img src="/public/images/how-to-guides/sql_studio/user_sso_or_oauth_connect_button.png" alt="Connect to Service Dialog" />
...
Basic Authentication requires you to manually enter credentials such as Username + Password, Service Account JSON, or Key Pair. Credentials are encrypted and stored securely.
...
**Option B: Username + Key Pair (Private Key + Passphrase)**
- Snowflake Username
...
<img src="/public/images/how-to-guides/sql_studio/user_basic_auth.png" alt="Snowflake Basic Authentication Form" />
...
## Trino / Redshift / PostgreSQL / MSSQL Basic Authentication
...
1. In SQL Studio, select the database service (Trino, Redshift, PostgreSQL, or MSSQL) from the dropdown
2. Click **Configure Connection**

#### Step 2: Enter Credentials

1. **Username**: Enter your database username (required)
2. **Password**: Enter your database password (required)
...
**For Redshift, PostgreSQL, MSSQL**:
- Verify username and password are correct
- Check that the database server is accessible from Collate
...
**For Redshift**:
- Verify user has `SELECT` permission on the target schemas/tables
- Check cluster security groups allow connections from Collate

**For PostgreSQL**:
- Verify user has `SELECT` permission on the target schemas/tables
- Check `pg_hba.conf` allows connections from Collate

**For MSSQL**:
- Verify user has `SELECT` permission on the target databases/schemas
- Check firewall rules allow connections from Collate
...
2. Each service shows:
   - Service name and type (BigQuery, Trino, Snowflake, Redshift, PostgreSQL, MSSQL)
