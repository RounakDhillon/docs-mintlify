---
title: BigQuery Connection | Collate BigQuery
...
sidebarTitle: BigQueryConnection
---
...
- **`credentials`**: GCP Credentials. These control **which BigQuery projects Collate reads metadata from** (data access). Refer to *[../../../../security/credentials/gcpCredentials.json](#/../../../security/credentials/gcpCredentials.json)*.
...
- **`billingProjectId`** *(string)*: The GCP project that will be **charged for the BigQuery jobs** Collate runs (metadata, usage, and lineage queries). This is separate from the `projectId` in your GCP credentials, which controls which datasets Collate scans. In simple single-project setups, leave this blank — BigQuery charges will default to the data project. Set it explicitly when your organization uses a centralized billing project or a shared service account that spans multiple data projects. The service account must have `bigquery.jobs.create` in this billing project. Default: `null`.
