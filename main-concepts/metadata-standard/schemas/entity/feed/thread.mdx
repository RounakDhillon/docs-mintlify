...
- **`id`**: Unique identifier that identifies an entity instance. Refer to *[../../type/basic.json](../../type/basic.json)*.
- **`type`**: Refer to *[#/definitions/threadType](threadType)*.
- **`href`**: Link to the resource corresponding to this entity. Refer to *[../../type/basic.json](../../type/basic.json)*.
- **`threadTs`**: Timestamp of the first post created the thread in Unix epoch time milliseconds. Refer to *[../../type/basic.json](../../type/basic.json)*.
- **`about`**: Data asset about which this thread is created for with format `<#E::{entities}::{entityName}::{field}::{fieldValue}>`. Refer to *[../../type/basic.json](../../type/basic.json)*.
...
- **`domain`**: Domain the entity belongs to. Refer to *[../../type/basic.json](../../type/basic.json)*.
...
- **`updatedAt`**: Last update time corresponding to the new version of the entity in Unix epoch time milliseconds. Refer to *[../../type/basic.json](../../type/basic.json)*.
...
- **`posts`** *(array)*
  - **Items**: Refer to *[#/definitions/post](post)*.
- **`reactions`**: Reactions for the thread. Refer to *[../../type/reaction.json](../../type/reaction.json)*.
- **`task`**: Details about the task. This is only applicable if thread is of type task. Refer to *[#/definitions/taskDetails](taskDetails)*.
- **`announcement`**: Details about the announcement. This is only applicable if thread is of type announcement. Refer to *[#/definitions/announcementDetails](announcementDetails)*.
- **`chatbot`**: Details about the Chatbot conversation. This is only applicable if thread is of type Chatbot. Refer to *[#/definitions/chatbotDetails](chatbotDetails)*.
## Definitions
