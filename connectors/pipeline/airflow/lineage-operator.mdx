...
The Lineage Operator can be directly installed to the Airflow instances as part of the usual Collate Python
distribution:
...
Where `x.y.z` is the version of your Collate server, e.g., 0.13.0. It is important that server and client
versions match.
...
the parameters to connect to Collate.

Go to the Airflow UI > Admin > Connection and create a new `OpenMetadata` connection as follows:

<img src="/public/images/connectors/airflow/airflow-connection.png" alt="Airflow Connection" />
...
If the Collate server connection needs to happen through HTTPS, update the `Schema` accordingly to `https`.
