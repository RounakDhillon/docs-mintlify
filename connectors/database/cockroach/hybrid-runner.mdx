...
slug: /connectors/database/cockroach/hybrid-runner
...
Configure and schedule Cockroach metadata workflows from the Collate UI:
...
## Requirements
To use the Cockroach connector with the Hybrid Ingestion Runner, ensure:
- You have a running CockroachDB cluster (self‑hosted or cloud) reachable from the Hybrid Runner environment.
- Network access (host, port, and any VPN/firewall rules) allows the Hybrid Runner to connect to Cockroach.
- A database user exists with sufficient read privileges to list databases, schemas, tables, views, and columns.
- TLS and certificates are configured, if required by your Cockroach deployment.
...
<Columns cols={2}>
  <Card
    title="Cockroach Troubleshooting"
    href="/connectors/database/cockroach/troubleshooting">
    Learn more about how to troubleshoot common Cockroach connector issues and resolve configuration or ingestion errors.
  </Card>
</Columns>
