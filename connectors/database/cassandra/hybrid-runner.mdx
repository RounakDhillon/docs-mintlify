...
slug: /connectors/database/cassandra/hybrid-runner
...
- **Host Port**: When using the `cassandra` connection schema, the `hostPort` parameter specifies the Cassandra host and port. Provide it as a string in the format `hostname:port`, for example `localhost:9042`.
- **databaseName**: Optional name to give to the database in OpenMetadata. If left blank, we will use the default database name.
...
<Columns cols={2}>
  <Card
    ...
    href="/connectors/database/cassandra/troubleshooting">
