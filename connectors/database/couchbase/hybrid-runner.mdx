...
slug: /connectors/database/couchbase/hybrid-runner
...
- **Hostport**: If Couchbase is hosted in the cloud, the `hostport` parameter specifies the connection string. If you are using Couchbase Server, it specifies the Couchbase hostname. This should be provided as a string in the format `hostname` or `xyz.cloud.couchbase.com`, for example `localhost`.
- **bucketName**: Optional name to give to the bucket in OpenMetadata. If left blank, we will ingest all bucket names.
</Step>
...
<Columns cols={2}>
  <Card
    title="Couchbase Troubleshooting"
    href="/connectors/database/couchbase/troubleshooting">
    Learn more about how to troubleshoot common Couchbase connector issues and resolve configuration or ingestion errors.
  </Card>
</Columns>
