---
title: DynamoDB Connector | Collate NoSQL Database Integration
description: Connect DynamoDB to Collate with our comprehensive database connector guide. Setup instructions, configuration options, and metadata extraction steps.
...
Configure and schedule DynamoDB metadata workflows from the Collate UI:
...
OpenMetadata retrieves information about all tables in the AWS account, describes each table to extract its schema, and scans a sample of records to infer column types. The user must have the following permissions:
- `dynamodb:ListTables` — to discover all tables in the account.
- `dynamodb:DescribeTable` — to retrieve table metadata (key schema, attribute definitions, item count, etc.).
- `dynamodb:Scan` — to sample table data for schema inference and profiling.

Below defined policy grants the necessary permissions:
```json
{
    "Version": "2012-10-17",
    "Statement": [
...
## Metadata Ingestion
<MetadataIngestionUi connector={"DynamoDB"} selectServicePath={"/public/images/connectors/dynamodb/select-service.png"} addNewServicePath={"/public/images/connectors/dynamodb/add-new-service.png"} serviceConnectionPath={"/public/images/connectors/dynamodb/service-connection.png"} />
## Connection Details
<Steps>
